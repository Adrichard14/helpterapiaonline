<?php
    // include_once("pagseguro2/PagSeguroLibrary.php");
    class TransactionAppointment {
        ...
        public $PUBLIC_VARS = array("ID", "pagseguroURL", "status", "payment_status", "clientID", "eventID", "cost", "status_date", "registry_date", "cancel_cause");
        public static $DATABASE = "transactions_appointment";
        public static $PAYMENT_STATUS_LIST  = array("pendente", "aprovado", "em disputa", "estornado", "cancelado");
        public static $SHIP_STATUS_LIST     = array("aguardando pagamento", "confirmada", "consulta cancelada");
        ...
        public static function load($ID = -1, $limit = NULL, $orderby = NULL, $status = -1, $payment_status = -1, $clientID = -1, $eventID = -1) {
        ...
        public static function create($clientID, $eventID, $payment_status = 0, $status = 1) {
            $client = $clientID > 0 ? Client::load($clientID, "0,1", NULL, NULL, false, -1) : array();
            ...
            $insert = $con->query(
                ...
                array("diiii", floatval($cost), intval($clientID), intval($eventID), intval($payment_status), intval($status)%2),
                false
            );
            if($insert) {
                $values = $con->query(
                    "SELECT * FROM `".self::$DATABASE."` WHERE `clientID`=? AND `eventID`=? ORDER BY `registry_date` DESC LIMIT 0,1",
                    array("ii", intval($clientID), intval($eventID))
                );
                if(!empty($values)) {
                    $ID = $values[0]['ID'];
                    ...
                    $con->query(
                        ...
                        array("si", $pagseguroURL."", intval($ID)),
                        false
                    );
                    return array("URL" => $pagseguroURL, "transactionID" => $ID, "status" => 1);
                }
            }
        ...
        public static function set_on_pagseguro(array $elem) {
            ...
            if($elem['pagseguroURL'] == "") {
                    ...
                    $request->addParameter('notificationURL', PUBLIC_URL. 'transaction-notify-appointment.php?ID='.$elem['ID'].'&pID='.$elem['eventID'].'&uID='.$elem['clientID']);
                    ...
                    try {
                        ...
                        $credentials = new PagSeguroAccountCredentials('helpterapiaonline@gmail.com', 'bfd9d68d-e949-428a-9302-687039b7176b5144c01547ae90a9f21d7c685da411677ff9-56a1-47c4-ab9e-19bcf72d88a6');
        ...
        public static function update_status($ID, $payment_status, $cancel_cause = "") {
            ...
            $update = Connector::newInstance()->query(
                ...
                array("sii", $cancel_cause."", intval($payment_status), intval($ID)),
                false
            );
            if($update) {
