<?php
    ...
    class Transaction {
        ...
        public $PUBLIC_VARS = array("ID", "pagseguroURL", "status", "payment_status", "customerID", "planID", "cost", "status_date", "registry_date", "cancel_cause");
        ...
        public static function load($ID = -1, $limit = NULL, $orderby = NULL, $status = -1, $payment_status = -1, $customerID = -1, $planID = -1) {
        ...
        public static function create($customerID, $planID, $payment_status = 0, $status = 1) {
            $customer = $customerID > 0 ? Psychologist::load($customerID, "0,1", NULL, NULL, false, -1) : array();
            ...
            $insert = $con->query(
                ...
                array("diiii", floatval($cost), intval($customerID), intval($planID), intval($payment_status), intval($status)%2),
                false
            );
            if($insert) {
                $values = $con->query(
                    "SELECT * FROM `".self::$DATABASE."` WHERE `customerID`=? AND `planID`=? ORDER BY `registry_date` DESC LIMIT 0,1",
                    array("ii", intval($customerID), intval($planID))
                );
                if(!empty($values)) {
                    $ID = $values[0]['ID'];
        ...
        public static function set_on_pagseguro(array $elem) {
            ...
            if($elem['pagseguroURL'] == "") {
                    ...
                    $request->addParameter('notificationURL', PUBLIC_URL.'transaction-notify.php?ID='.$elem['ID'].'&pID='.$elem['planID'].'&uID='.$elem['customerID']);
