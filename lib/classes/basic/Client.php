<?php
	...
	class Client {
		...
		private $sexo;
		private $cpf;
		private $PRIVATE_VARS = array('ID', 'login', 'password', 'name', 'nickname', 'mail', 'access_token', 'recovery_token', 'recovery_validation','registry_date', 'thumb', 'telefone', 'data_nasc', 'sexo', 'cpf');
		public $PUBLIC_VARS = array('ID', 'name', 'nickname', 'access_token');
        public static $ABSOLUTE_USERS   = array(1,2);
        public static $DATABASE         = "clients";
        public static $SESSION          = "l@#$@e@#r";
		public static $FOLDER           = "uploads/clients";
		...
		public static function create($login, $password, $password2, $name, $nickname, $mail, $thumb, $telefone, $data_nasc, $sexo, $cpf) {
			//  if($thumb == "")
			...
			if($con->query(
				"INSERT INTO `".self::$DATABASE."` (login, password, name, nickname, `mail`, thumb, telefone, data_nasc, sexo, cpf) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
				array("ssssssssss", $login."", $password."", $name."", $nickname."", $mail."", $thumb."", $telefone . "", $data_nasc ."", $sexo ."", $cpf . ""),
				false
			)) {
				$u = $con->query(
					"SELECT ID FROM `".self::$DATABASE."` WHERE mail=?",
					array("s", $mail."")
				);
				return $u[0]["ID"];
			}
		...
		public static function update($ID, $login, $password, $password2, $name, $nickname, $mail, $thumb, $telefone, $data_nasc, $sexo, $cpf) {
			...
			$set = " SET login=?, name=?, nickname=?, `mail`=?, `thumb`=?, `telefone`=?, `data_nasc`=?, `sexo`=?, `cpf`=?";
			$args = array("sssssssss", $login."", $name."", $nickname."", $mail."", $thumb ."", $telefone . "", $data_nasc ."", $sexo . "", $cpf."");
			if($password != "") {
				if(!Format::isPassword($password))
                    exit(INVALID_PASSWORD);
				if($password !== $password2)
...
//            }
        ...
        public static function generate_recovery_token($mail) {
            ...
            if(!empty($values)) {
               ...
               $rURL = PUBLIC_URL . "nova-senha-user";
