<?php
    class Plans {
        ...
        public $PUBLIC_VARS = array("ID", "status","order", "price", "thumb","title", "status_date", "init_date", "end_date", "registry_date","content");
        public static $DATABASE         = "basic_plans";
        public static $FOLDER           = "uploads/planos";
        ...
        public static function load($ID = -1, $limit = NULL, $orderby = 'title ASC', $status = -1, $ondate = NULL) {
            $w = $a = NULL;
            self::prepareSearch($w, $a, $ID, $limit, $orderby, $status, $ondate);
            // exit(var_dump("SELECT * FROM `".self::$DATABASE."`".$w, $a));
        ...
        private static function prepareValues($input, &$output) {
            $vars = array("thumb","price", "title", "init_date", "end_date", "status","content","order");
            foreach($vars as $var) {
                if(!isset($input[$var]))
        ...
        public static function create($FAKE_POST = NULL) {
            ...
            $insert = Connector::newInstance()->query(
                "INSERT INTO `".self::$DATABASE."` (`thumb`,`title`,`content`, `init_date`, `end_date`, `price`, `status`, `order`, `status_date`) VALUES (?,?, ?, ?, ?, ?, ?, ?, NOW())",
                array("sssssdii", $output["thumb"]."", $output["title"]."", $output["content"]."", $output["init_date"]."", $output["end_date"]."", floatval($output["price"]), intval($output["status"]), intval($output["order"])),
                false
            );
            if($insert)
        ...
        public static function update($FAKE_POST = NULL) {
            ...
            $update = Connector::newInstance()->query(
                "UPDATE `".self::$DATABASE."` SET `thumb`=?,`title`=?,`content`=?, `init_date`=?, `end_date`=?, `price`=?, `status`=?, `status_date`=NOW() WHERE `ID`=?",
                array("sssssdii", $output["thumb"]."",$output["title"]."",$output["content"]."", $output["init_date"]."", $output["end_date"]."", floatval($output["price"]), intval($output["status"]), intval($input['ID'])),
                false
            );
            if($update) {
                $base = "";
