<?php
    class Workhours {
        ...
        public $PUBLIC_VARS = array("ID", "status","title","registry_date", "date", "hour", "workerID");
        public static $DATABASE         = "basic_workerhours";
        ...
        private static function prepareSearch(&$w, &$a, $ID, $limit, $orderby, $status, $date, $hour, $workerID) {
            $h = $ID != -1 || $status != -1 || $date != NULL || $hour != NULL || $workerID != -1;
            $w = $h ? " WHERE" : "";
        ...
        public static function load($ID = -1, $limit = NULL, $orderby = NULL, $status = -1, $date = NULL, $hour = NULL, $workerID = -1) {
            $w = $a = NULL;
            self::prepareSearch($w, $a, $ID, $limit, $orderby, $status, $date, $hour, $workerID);
        ...
        public static function create2($status, $date, $hour, $workerID) {
            $insert = Connector::newInstance()->query(
                "INSERT INTO `".self::$DATABASE. "` (`status`, `date`, `hour`, `workerID`) VALUES (?, ?, ?, ?)",
                array("issi", intval($status)%2, $date."", $hour."", intval($workerID)),
                false
            );
            if($insert) {
                return "Sucesso";
        ...
        public static function update($FAKE_POST = NULL) {
            ...
            $update = Connector::newInstance()->query(
                "UPDATE `".self::$DATABASE."` SET `title`=?, `status`=? WHERE `ID`=?",
                array("sii", $output["title"]."", intval($output["status"]), intval($input['ID'])),
                false
            );
            if($update) {
                $base = "";
