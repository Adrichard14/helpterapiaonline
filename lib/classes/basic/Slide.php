<?php
    class Slide {
        ...
        public $PUBLIC_VARS = array("ID", "status","order", "link", "thumb", "thumb_tablet", "thumb_mobile", "title", "status_date", "init_date", "end_date", "registry_date");
        public static $DATABASE         = "basic_slide";
        public static $FOLDER           = "uploads/slides";
        public static $DESKTOP          = array(1280,889);
        public static $TABLET           = array(980,889);
        ...
        private static function prepareValues($input, &$output) {
            $vars = array("thumb", "thumb_tablet", "thumb_mobile", "link", "title", "init_date", "end_date", "status","order");
            foreach($vars as $var) {
                if(!isset($input[$var]))
        ...
        public static function create($FAKE_POST = NULL) {
            ...
            $insert = Connector::newInstance()->query(
                "INSERT INTO `".self::$DATABASE."` (`thumb`, `thumb_tablet`, `thumb_mobile`, `title`, `init_date`, `end_date`, `link`, `status`, `order`, `status_date`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())",
                array("sssssssii", $output["thumb"]."", $output["thumb_tablet"]."", $output["thumb_mobile"]."", $output["title"]."", $output["init_date"]."", $output["end_date"]."", $output["link"]."", intval($output["status"]), intval($output["order"])),
                false
            );
            if($insert)
                return SLIDE_INSERTED;
            exit(SLIDE_INSERTION_FAILED);
        }
        public static function update($FAKE_POST = NULL) {
            ...
            $update = Connector::newInstance()->query(
                "UPDATE `".self::$DATABASE."` SET `thumb`=?, `thumb_tablet`=?, `thumb_mobile`=?, `title`=?, `init_date`=?, `end_date`=?, `link`=?, `status`=?, `status_date`=NOW() WHERE `ID`=?",
                array("sssssssii", $output["thumb"]."", $output["thumb_tablet"]."", $output["thumb_mobile"]."", $output["title"]."", $output["init_date"]."", $output["end_date"]."", $output["link"]."", intval($output["status"]),intval($input['ID'])),
                false
            );
            if($update) {
                $base = "";
