...
class Slim {
    ...
    private static function parseInput($value) {
        ...
        if (isset($data["actions"])) {
            $actions = array(
                'crop' => isset($data["actions"]["crop"]) ? $data["actions"]["crop"] : null,
                'size' => isset($data["actions"]["size"]) ? $data["actions"]["size"] : null,
                'rotation' => $data["actions"]["rotation"],
                'filters' => isset($data["actions"]["filters"]) ? $data["actions"]["filters"] : null
            );
        }
    ...
    public static function saveFile($data, $name, $path = 'images/', $uid = true) {
        $base = '';
        while(!file_exists($base."lib"))
            $base.="../";
        $path = $base.'uploads/'.$path;
...
//            return null;
//        }
        return file_get_contents($url, false, null, 0, $maxFileSize);
    }
