<?php
	...
	while(!$ready) {
		...
		$list = Doctor::load(-1, $page_limit, "`registry_date` DESC", 2.0);
		$ready = !empty($list) || $page == 1;
		if(!$ready) $page--;
	}
	$paginator = new Paginator(Doctor::paginator(2.0), "doctors.php?page=", $page, $limit);
	$headers = array(
		...
		array("value" => "Especialidade", "class" => "text-center"),
	...
	if(empty($list))
		$table->addRow(
			array(
				array("value" => "Não há dados para exibir.".str_replace("{LINK}", 'write-doctor.php', ' '.CLICK_HERE_TO_WRITE_ONE), "class" => "text-center", "colspan" => sizeof($headers))
			)
		);
	else foreach($list as $pos => $i) {
        $menu = array(
			'write-doctor.php?ID='.$i["doctorID"] => '<i class="clip-pencil-3"></i> '.UPDATE
        );
		$menu = array_merge($menu, array(
			'javascript:this.toggle(\'Doctor\',  '.$i["doctorID"].')' => '<i class="icon-off"></i> '.TOGGLE,
			'javascript:this.del(\'Doctor\', '.$i["doctorID"].')' => '<i class="icon-remove"></i> '.DELETE
		));
		$row = array(
			array("value" => Format::getMenu($menu), "class" => "text-center"),
			array("value" => '#'.$i['doctorID'], "class" => "text-center"),
			array("value" => $sts[$i['status']], "class" => "text-center"),
			array("value" => $i['name'], "class" => "text-center"),
			array("value" => $i['specialty'], "class" => "text-center"),
			array("value" => $i['URL'], "class" => "text-center"),
	...
	$nav = "list-doctors";
	$page_title = LIST_OF."médicos".' <a href="write-doctor.php"><i class="clip-plus-circle-2 text-success clickable" title="'.INSERT.'"></i></a>';
	$page_desc = NULL;
?>
<!DOCTYPE html>
