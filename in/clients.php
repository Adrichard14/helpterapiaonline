...
while (!$ready) {
    ...
    $list = Client::load(-1, $page_limit, "`registry_date` DESC");
    $ready = !empty($list) || $page == 1;
    if (!$ready) $page--;
}
$paginator = new Paginator(sizeof(Client::load()), "clients.php?page=", $page, $limit);
$headers = array(
...
if (empty($list))
    $table->addRow(
        array(
            array("value" => USER_EMPTY_TABLE . str_replace("{LINK}", '#" onclick="newClient()', ' ' . CLICK_HERE_TO_WRITE_ONE), "class" => "text-center", "colspan" => sizeof($headers))
        )
    );
else foreach ($list as $i) {
    $menu = array(
        'javascript:updateClient(' . $i["ID"] . ')' => '<i class="clip-pencil-3"></i> ' . UPDATE,
        'javascript:deleteClient(' . $i["ID"] . ')' => '<i class="icon-remove"></i> ' . DELETE . ' ' . strtolower(USER)
    );
    $row = array(
...
$nav = "list-clients";
$page_title = LIST_OF . 'clientes' . ' <i onClick="newClient()" class="clip-plus-circle-2 text-success clickable" title="' . INSERT . ' ' . strtolower(USER) . '"></i>';
$page_desc = NULL;
?>
<!DOCTYPE html>
...
<body><?php include_once("inc/header.php"); ?>
    ...
    <!-- end: MAIN CONTAINER --><?php include_once("inc/footer.php");
                                Page::script("assets/js/in/Client.js"); ?>
</body>
<!-- end: BODY -->
