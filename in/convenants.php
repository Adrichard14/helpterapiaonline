<?php
	...
	while(!$ready) {
		...
		$list = Convenant::load(-1, $page_limit, "`order` ASC", 2.0);
		$ready = !empty($list) || $page == 1;
		if(!$ready) $page--;
	}
	$paginator = new Paginator(Convenant::paginator(2.0), "convenants.php?page=", $page, $limit);
	$headers = array(
	...
	if(empty($list))
		$table->addRow(
			array(
				array("value" => "Não há dados para exibir.".str_replace("{LINK}", 'write-convenant.php', ' '.CLICK_HERE_TO_WRITE_ONE), "class" => "text-center", "colspan" => sizeof($headers))
			)
		);
	else foreach($list as $pos => $i) {
        $menu = array(
			'write-convenant.php?ID='.$i["ID"] => '<i class="clip-pencil-3"></i> '.UPDATE,
            PUBLIC_URL.$i['thumb'].'" title="#'.$i['ID'].'" class="fancybox" rel="gallery' => '<i class="clip-eye"></i> Visualizar imagem'
        );
		$menu = array_merge($menu, array(
			'javascript:this.toggle(\'Convenant\',  '.$i["ID"].')' => '<i class="icon-off"></i> '.TOGGLE,
			'javascript:this.del(\'Convenant\', '.$i["ID"].')' => '<i class="icon-remove"></i> '.DELETE
		));
		$row = array(
	...
	$nav = "list-convenant";
	$page_title = LIST_OF."convênios".' <a href="write-convenant.php"><i class="clip-plus-circle-2 text-success clickable" title="'.INSERT.'"></i></a>';
	$page_desc = NULL;
?>
<!DOCTYPE html>
...
<html lang="pt-br" class="no-js">
	...
	<body><?php include_once("inc/header.php"); ?>
		...
		<!-- end: MAIN CONTAINER --><?php include_once("inc/footer.php"); Page::script("assets/js/in/convenants.js"); ?>
	</body>
	<!-- end: BODY -->
</html>