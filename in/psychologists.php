...
while (!$ready) {
    ...
    $list = Psychologist::load(-1, $page_limit, "`registry_date` DESC");
...
if (empty($list))
    ...
    $menu = array(
        'javascript:updatePsychologist(' . $i["ID"] . ')' => '<i class="clip-pencil-3"></i> ' . UPDATE,
        'javascript:deletePsychologist(' . $i["ID"] . ')' => '<i class="icon-remove"></i> ' . DELETE . ' ' . strtolower(USER)
    );
    $row = array(
