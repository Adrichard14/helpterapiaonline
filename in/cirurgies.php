<?php
	...
	while(!$ready) {
		...
		$list = Cirurgy::load(-1, $page_limit, "`order` ASC", 2.0);
		$ready = !empty($list) || $page == 1;
		if(!$ready) $page--;
	}
	$paginator = new Paginator(Cirurgy::paginator(2.0), "cirurgies.php?page=", $page, $limit);
	$headers = array(
	...
	if(empty($list))
		$table->addRow(
			array(
				array("value" => "Não há dados para exibir.".str_replace("{LINK}", 'write-cirurgy.php', ' '.CLICK_HERE_TO_WRITE_ONE), "class" => "text-center", "colspan" => sizeof($headers))
			)
		);
	else foreach($list as $pos => $i) {
        $menu = array(
			'write-cirurgy.php?ID='.$i["ID"] => '<i class="clip-pencil-3"></i> '.UPDATE,
            PUBLIC_URL.$i['thumb'].'" title="#'.$i['ID'].'" class="fancybox" rel="gallery' => '<i class="clip-eye"></i> Visualizar imagem'
        );
		$menu = array_merge($menu, array(
			'javascript:this.toggle(\'Cirurgy\',  '.$i["ID"].')' => '<i class="icon-off"></i> '.TOGGLE,
			'javascript:this.del(\'Cirurgy\', '.$i["ID"].')' => '<i class="icon-remove"></i> '.DELETE
		));
		$row = array(
	...
	$nav = "list-cirurgy";
	$page_title = LIST_OF."cirurgias".' <a href="write-cirurgy.php"><i class="clip-plus-circle-2 text-success clickable" title="'.INSERT.'"></i></a>';
	$page_desc = NULL;
?>
<!DOCTYPE html>
...
<html lang="pt-br" class="no-js">
	...
	<body><?php include_once("inc/header.php"); ?>
		...
		<!-- end: MAIN CONTAINER --><?php include_once("inc/footer.php"); Page::script("assets/js/in/cirurgies.js"); ?>
	</body>
	<!-- end: BODY -->
</html>