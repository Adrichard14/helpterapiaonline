...
while (!$ready) {
    ...
    $list = FaqAdvantage::load(-1, $page_limit, "`order` ASC", 2.0);
    $ready = !empty($list) || $page == 1;
    if (!$ready) $page--;
}
$paginator = new Paginator(FaqAdvantage::paginator(2.0), "faqs.php?page=", $page, $limit);
$headers = array(
...
if (empty($list))
    $table->addRow(
        array(
            array("value" => "Não há dados para exibir." . str_replace("{LINK}", 'write-faq.php', ' ' . CLICK_HERE_TO_WRITE_ONE), "class" => "text-center", "colspan" => sizeof($headers))
        )
    );
else foreach ($list as $pos => $i) {
    $menu = array(
        'write-faqadvantage.php?ID=' . $i["ID"] => '<i class="clip-pencil-3"></i> ' . UPDATE,   
    );
    $menu = array_merge($menu, array(
        'javascript:this.toggle(\'Faq\',  ' . $i["ID"] . ')' => '<i class="icon-off"></i> ' . TOGGLE,
        'javascript:this.del(\'Faq\', ' . $i["ID"] . ')' => '<i class="icon-remove"></i> ' . DELETE
    ));
    $row = array(
        ...
        array("value" => $i['question'], "class" => "text-center"),
...
$nav = "list-advantage";
$page_title = LIST_OF . "Perguntas e respostas (Vantagens)" . ' <a href="write-faqadvantage.php"><i class="clip-plus-circle-2 text-success clickable" title="' . INSERT . '"></i></a>';
$page_desc = NULL;
?>
<!DOCTYPE html>
...
<body><?php include_once("inc/header.php"); ?>
    ...
    <!-- end: MAIN CONTAINER --><?php include_once("inc/footer.php");
                                Page::script("assets/js/in/faqadvantages.js"); ?>
</body>
<!-- end: BODY -->
