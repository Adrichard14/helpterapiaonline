...
while (!$ready) {
    ...
    $list = Transaction::load(-1, $page_limit, "`order` ASC", 2.0);
    $ready = !empty($list) || $page == 1;
    if (!$ready) $page--;
}
$paginator = new Paginator(Transaction::paginator(2.0), "transactions.php?page=", $page, $limit);
$headers = array(
...
if (empty($list))
    $table->addRow(
        array(
            array("value" => "Não há dados para exibir." . str_replace("{LINK}", 'write-transaction.php', ' ' . CLICK_HERE_TO_WRITE_ONE), "class" => "text-center", "colspan" => sizeof($headers))
        )
    );
    ...
    $menu = array_merge($menu, array(
        'javascript:this.toggle(\'Transaction\',  ' . $i["ID"] . ')' => '<i class="icon-off"></i> ' . TOGGLE,
        'javascript:this.del(\'Transaction\', ' . $i["ID"] . ')' => '<i class="icon-remove"></i> ' . DELETE
    ));
    $row = array(
...
$page_title = LIST_OF . "transações" . ' <a href="write-transaction.php"><i class="clip-plus-circle-2 text-success clickable" title="' . INSERT . '"></i></a>';
$page_desc = NULL;
?>
<!DOCTYPE html>
